...
//demo for custom add camera drive

//#define CAM_POW_CTRL_GPIO
//#define CAM_POW_CTRL_LDO
...
#define USER_CAM_PWDN  SC_MODULE_GPIO_28
#define USER_CAM_RST   SC_MODULE_GPIO_29
#define USER_CAM_AVDD  SC_MODULE_GPIO_8
#define USER_CAM_IOVDD SC_MODULE_GPIO_17
#endif

#ifdef CAM_POW_CTRL_LDO
#define USER_CAM_PWDN  SC_MODULE_GPIO_10
#define USER_CAM_RST   SC_MODULE_GPIO_20
#endif
...
int GC032a_s_power(int on)
{
	...
	if (on > 0) {
		sAPI_GpioSetValue(USER_CAM_PWDN, 0);
		sAPI_TaskSleep(2);
		sAPI_CamLdoOnOff(0,CAMERA_AVDD);
		sAPI_TaskSleep(2);
		sAPI_CamLdoOnOff(0,CAMERA_IOVDD);
		sAPI_TaskSleep(2);
		sAPI_CamMclkOnOff(0);
		sAPI_TaskSleep(1); //delay 5ms
		sAPI_CamLdoOnOff(1,CAMERA_IOVDD);
		sAPI_TaskSleep(1);
		sAPI_CamLdoOnOff(1,CAMERA_AVDD);
		sAPI_TaskSleep(1);
		sAPI_CamMclkOnOff(1);
		sAPI_TaskSleep(1);
		sAPI_GpioSetValue(USER_CAM_PWDN, 1);
