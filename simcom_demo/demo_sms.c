...
INT8 input_data[SC_SMS_MAX_MULTIPLE_MSG_LENGTH + 1];
...
BOOL inputDataProcessor(INT8 *data_in, UINT32 data_in_len)
{
     ...
     if (TRUE != gSmsDemohasinited)
     {
        ...
        /*msgQ prepared for urc processor*/
         ...
         if(NULL == gSmsUrcProcessTask){
              ...
              if(sAPI_TaskCreate(&gSmsUrcProcessTask, gSmsUrcProcessTaskStack, 1024*4, 90, (char*)"SmsurcProcesser", sTask_SmsUrcProcessor,(void *)0))
             {
...
SC_SMSReturnCode smsMsgWritedemo(void)
{
    ...
    while(1)
    {
        ...
        sAPI_UartWriteString(SC_UART, input_data);
    ...
    while(1)
    {
        ...
        switch(opt)
        {
            ...
            case SC_SMS_DEMO_SIMPLE_TESET:
            {
                ...
                ret = sAPI_SmsSendMsg( 1, "123asdfd", strlen("123asdfd"), "13364028515", g_sms_demo_msgQ);
                if(ret == SC_SMS_SUCESS)
                {
                    memset(&msg,0,sizeof(msg));
                    status = sAPI_MsgQRecv(g_sms_demo_msgQ,&msg,SMS_URC_RECIVE_TIME_OUT);
                ...
                sAPI_UartWriteString(SC_UART, (UINT8*)"\t\r\n2. Send pdu sms test.");
                ret = sAPI_SmsSendMsg( 0, "0001000D91683163048215F500080C00310032003300344F60597D", 26, NULL, g_sms_demo_msgQ);
                sAPI_Debug("%s, %d, msg write, ret=%d!!",__func__, __LINE__, ret);
                if(ret == SC_SMS_SUCESS)
                {
                    memset(&msg,0,sizeof(msg));
                ...
                ret = sAPI_SmsWriteMsg( 1, "123asd", strlen("123asd"), "13364028515", 1, g_sms_demo_msgQ);
                sAPI_Debug("%s, %d, msg write, ret=%d!!",__func__, __LINE__, ret);
                if(ret == SC_SMS_SUCESS)
                {
                    memset(&msg,0,sizeof(msg));
                ...
                ret = sAPI_SmsWriteMsg( 0, "0001000D91683163048215F500080C00310032003300344F60597D", strlen("0001000D91683163048215F500080C00310032003300344F60597D"), NULL, 1, g_sms_demo_msgQ);
                sAPI_Debug("%s, %d, msg write, ret=%d!!",__func__, __LINE__, ret);
                if(ret == SC_SMS_SUCESS)
                {
                    memset(&msg,0,sizeof(msg));
                ...
                ret = sAPI_SmsSendStrogeMsg( 2, "15730119169", g_sms_demo_msgQ);
