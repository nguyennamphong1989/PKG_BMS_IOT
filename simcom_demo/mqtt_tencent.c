...
void* Get_Cfg_Parameters(char *product_id,char *device_name,char *device_secret,char *region,  char *cfg_client_id,char *cfg_host_addr,char *cfg_username, char *cfg_password)
{
    ...
    sAPI_Debug("func[%s] line[%d] MQTT get cfg client_id:%s", __FUNCTION__,__LINE__,client_id);
    ...
    sAPI_Debug("func[%s] line[%d] MQTT get cfg host_addr:%s", __FUNCTION__,__LINE__,host_addr);
    ...
    memset(cfg_username,0,sizeof(cfg_username));
    snprintf(cfg_username,100,"%s",username);
